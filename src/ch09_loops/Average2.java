...
public class Average2 {
	...
	public static double avgWithDoWhileLoop() {
		double sum = 0;
		int flag = -1;
		int no_tests = 0;
		double points;
		do {
			points = Double.parseDouble(JOptionPane.showInputDialog(
					"Test "+(no_tests+1)+": negative value to stop the cycle."));
			if (points > flag) {
				sum += points;
				no_tests++;
			}
		...
		if (no_tests > 0) {
			System.out.println("Number of tests is "+no_tests);
			return sum/no_tests;
		}
		else System.out.println("No tests.");
		return 0;
	}
	
	public static void main(String[] args) {
		double avg = avgWithDoWhileLoop();
		System.out.printf("Test average is %.2f %n", avg);
	}
